 ...
 <div id="load_view" class="col-md-12">
               <form action="<?=base_url('/master/index/tbl_x_ray');?>" method="post" id="serch_data" >
      ...
      <?php echo form_dropdown('Department', $departments, $Department, 'class="form-control" id="Department"'); ?>    
       ...
       <button type="button" class="btn btn-primary pull-right" id="print_list" style="padding-left:18px; padding-right:18px;">Print <i class="fa fa-print prntIcn" aria-hidden="true"></i></button>
...
var url = "<?=base_url('/master/print_data/tbl_x_ray');?>";
            ...
            <div class="clearfix"></div>
              <div class="card">
                ...
                <div class="card-header card-header-primary">
                  <h4 class="card-title ">X-Ray Status</h4>
                  <p class="card-category">Today X-ray</p>
                </div>
                <div class="card-body">
                    ...
                    <?php if($listings) :?>
                      ...
                      <?php $count = 1; foreach($listings as $listing): //print_r($listing); exit;?>
                        ...
                        <tr id="row_<?=$listing->ID;?>">
                          ...
                          <td>
                          <?=date('d/m/Y', strtotime($listing->xdate));?>
                          </td>
                          <td>
                          <?=$listing->PName;?>
                          </td>
...
var url = "<?=base_url('/master/print_data/tbl_x_ray/');?>" + $(this).attr('id');
  ...
  $('.pagination').on('click','a',function(e){
     ...
     <?php if($postcrno){ ?>
        ...
        $('.add_item').click(function(){
          $("#load_view").load("<?=base_url('/master/update/tbl_x_ray');?>", function(data){
            <?php if($postcrno){ ?>
    ...
    $('.edit_item').click(function(){ 
       $("#load_view").load("<?=base_url('/master/update/tbl_x_ray/');?>" + $(this).attr('id')); 
    });
   $('.delete_item').click(function(){ 
      if(confirm("Are you sure?")){
       $.get("<?=base_url('/master/delete/tbl_x_ray/');?>" + $(this).attr('id'), function(data){
