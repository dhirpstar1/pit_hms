<div class="col-md-12 fetch_results">
              <div class="card">
                <div class="col-md-12">
                  ...
                  <h4 class="card-title ">Physiotherapy</h4>
                  <p class="card-category"> Physiotherapy Register Master</p>
                </div>
                <br>
                <form action="<?=base_url('/master/update');?>" method="post" id="save_data">
                  ...
                  <input type="hidden" name="tbl" value="<?=$tbl;?>">
                    ...
                    <h5>Select Therapy</h5>
                    </div>
                    <?php  foreach($therapy as $key => $item): ?>
                    <div class="col-md-3">
                    <?=$item;?>
                    </div>
                    <div class="col-md-9">
                    <input type="text" class="form-control" name="name[<?=$key;?>]" value="<?php foreach($savephysio as $pkey => $pitem): if($pkey == $key){ echo $pitem;} endforeach;?>" >
                    </div>
                    <?php endforeach; ?>
                    </div>                  
