 ...
 <div id="load_view" class="col-md-12">
               <form action="<?=base_url('/master/index/tbl_panchkarma');?>" method="post" id="serch_data">
...
var url = "<?=base_url('/master/print_data/tbl_panchkarma');?>";
            ...
            <div class="clearfix"></div>
              <div class="card">
                <div class="card-header card-header-primary">
                  <h4 class="card-title ">Panchkarma Status</h4>
                  <p class="card-category">Today Panchkarma Status</p>
                </div>
                <div class="card-body">
                    ...
                    <?php if($listings) :?>
                      ...
                      <?php $count = 1; foreach($listings as $listing): //print_r($listing); exit;?>
                        ...
                        <tr id="row_<?=$listing->ID;?>">
                          ...
                          <td>
                          <?=($listing->ipdno > 0 ) ? date('d/m/Y', strtotime($listing->doa)) : 'NA';?>
                          </td>
                          <td>
                          <?=date('d/m/Y', strtotime($listing->pdate));?>
                          </td>
                           <td>
                          <?=$listing->pname;?>
                          </td>
  ...
  <script type="text/javascript">
   ...
   <?php if($postcrno){ ?>
        ...
        $('.add_item').click(function(){
          $("#load_view").load("<?=base_url('/master/update/tbl_panchkarma');?>", function(data){
            <?php if($postcrno){ ?>
    ...
    $('.edit_item').click(function(){ 
       $("#load_view").load("<?=base_url('/master/update/tbl_panchkarma/');?>" + $(this).attr('id')); 
    });
   $('.delete_item').click(function(){ 
      if(confirm("Are you sure?")){
       $.get("<?=base_url('/master/delete/tbl_panchkarma/');?>" + $(this).attr('id'), function(data){
